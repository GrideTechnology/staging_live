...
class MenuController extends Controller
{
        ...
        try{ 
            ...
            if($request->has('action') && $request->action=="delete") {  
                if($country_id =='ALL' || $country_id ==''){                
                    ...
                    \Log::info("City Id-----".json_encode($city_details));   
                    foreach($city_details as $city)
                    {     
                        $menucity = MenuCity::where('menu_id',$id)->where('country_id',$country_id);
                        
                        if($request->state_name){
                            $state_id = $request->state_name;
                            $menucity->where('state_id',$state_id);
                        }

                        $menucity->where('city_id',$city);
            ...
            if(count($city_details)>0){
                foreach($city_details as $city)
                {
                    ...
                    $isexist = MenuCity::where('menu_id', $id)
                                        ->where('country_id', $countryId->country_id)
                                        ->where('state_id', $countryId->state_id)
                                        ->where('city_id', $city)
                                        ->count();
                    if($isexist==0){
    ...
    {
        $vehicle_type_list = RideType::where('company_id', Auth::user()->company_id)->where('status',1)->where('is_deleted',0)->get();
