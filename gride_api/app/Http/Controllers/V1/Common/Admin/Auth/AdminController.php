...
class AdminController extends Controller
{
		...
		if($setting != null) {
			...
			if($request->has('provider_select_timeout')) {
				...
				if(!$request->has('geofence')) {
					$request->request->add(['geofence' => '0']);
				}
			...
			foreach($request->only(['user_select_timeout', 'provider_select_timeout', 'provider_search_radius', 'unit_measurement', 'credit_ride_limit', 'manual_request', 'broadcast_request', 'ride_otp', 'booking_prefix','geofence']) as $key => $req) {
			...
			$data['store_link_company_link'] = 	$request->store_link_company_link;
			
			if($request->has('store_link_company_link_enable')){
				$data['site']['store_link_company_link_enable'] = 	$request->store_link_company_link_enable;	
			}else{
				$data['site']['store_link_company_link_enable'] = 0;
			}
			...
			// $data['site']['country_code'] = 'us';
			$setting->settings_data = json_encode($data);
			$setting->save();

			if(!empty($request->adminservice) > 0 && $request->has('adminservice')){
	...
	public function appSettingStore(Request $request){
		try{
			$setting = Appsetting::find(1);
			$setting->android_user_version = $request->android_user_version;
			$setting->ios_user_version = $request->ios_user_version;
			$setting->android_provider_version = $request->android_provider_version;
			$setting->ios_provider_version = $request->ios_provider_version;
			$setting->save();
			return Helper::getResponse(['status' => 200,'data'=>$setting]);
		}
		catch (Exception $e) {
