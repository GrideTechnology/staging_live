	...
	*/
	...
	public function createRide(Request $request) {
		...
		if($this->settings->transport->geofence == 1) {						
			...
			$geofence = null;
			$ride_city_price = RideCityPrice::where('state_id',$this->user->state_id)->whereNull('city_id')->where('ride_delivery_vehicle_id', $request->service_type)->first();
		}
		...
		if(!empty($request->schedule_date)&&!empty($request->schedule_date)){
			if($request->estimated_fare < $ride_city_price->scheduled_minimum_fee){
				return ['status' => 400, 'message' => trans('api.ride.min_price_reject'), 'error' => trans('api.ride.min_price_reject')];
			}
		} else {
			if($request->estimated_fare < $ride_city_price->min_price){													
				return ['status' => 400, 'message' => trans('api.ride.min_price_reject'), 'error' => trans('api.ride.min_price_reject')];
			}
			if($request->estimated_fare > $ride_city_price->max_price){									
				return ['status' => 400, 'message' => trans('api.ride.max_price_reject'), 'error' => trans('api.ride.max_price_reject')];
			}
		}
		...
		try {
			...
			// echo '<pre>';print_r($details['routes'][0]['legs'][0]['distance']['value']/1000);exit;
			...
			$rideRequest->unit = isset($this->settings->site->distance) ? $this->settings->site->distance : 'MILES';
			\Log::warning($this->settings->site->distance);
			\Log::warning($rideRequest->unit);
			...
			\Log::info("Sending request to providers");
			(new UserServices())->createRequest([$Providers->first()], $rideRequest, 'TRANSPORT');
	...
	public function updateRide(Request $request) { 
		try{
			...
			$rideRequest = RideRequest::where('id', $request->id)->first();//with('user')->findOrFail($request->id)			
			...
			$request->request->add(['s_latitude' => $rideRequest->s_latitude]);
			$request->request->add(['s_longitude' => $rideRequest->s_longitude]);
			$request->request->add(['state_id' => $rideRequest->state_id]);
			$request->request->add(['service_type' => 1]);

			//$fare = (new UserServices())->estimated_fare($request)->getData();	
			//print_r($rideRequest); exit;
			if($request->status == 'DROPPED' && $request->d_latitude != null && $request->d_longitude != null) {	

				// $distanceAPI = 'https://maps.googleapis.com/maps/api/distancematrix/json?destinations='.$request->d_latitude.','.$request->d_longitude.'&origins='.$rideRequest->s_latitude.','.$rideRequest->s_longitude.'&key='.$siteConfig->server_key;
				
				// $json = Helper::curl($distanceAPI);

				// $distance = json_decode($json, TRUE);

				// $miles = explode(' ',$distance['rows'][0]['elements'][0]['distance']['text']);

				

						$addressAPI = 'https://maps.googleapis.com/maps/api/geocode/json?latlng='.$request->d_latitude.','.$request->d_longitude.'&key=AIzaSyDo5-R0tZH0cWEvjvaOXin8fOSKHb_t35Q';
						
						$json = Helper::curl($addressAPI);

						$address = json_decode($json, TRUE);
						
						$requestarr = ['s_address'=>$rideRequest->s_address, 'd_address'=>$address['results'][0]['formatted_address'], 'server_key'=>'AIzaSyDo5-R0tZH0cWEvjvaOXin8fOSKHb_t35Q'];

						$location1=$this->getDistanceByAddress($requestarr);

						// $rideRequest->distance = round($miles[0]/0.621371, 2);
						$rideRequest->d_latitude = $request->d_latitude;
						$rideRequest->d_longitude = $request->d_longitude;
						// $rideRequest->d_address = $request->d_address;

						$rideRequest->distance = round($location1['distance']/0.621371, 1);
						// $rideRequest->d_latitude = $location1['d_latitude'];
						// $rideRequest->d_longitude = $location1['d_longitude'];
						$rideRequest->d_address = $address['results'][0]['formatted_address']; //$request->d_address;
					//}
				// }

				// else{

				// 	$addressAPI = 'https://maps.googleapis.com/maps/api/geocode/json?latlng='.$request->d_latitude.','.$request->d_longitude.'&key='.$siteConfig->server_key;
						
				// 	$json = Helper::curl($addressAPI);

				// 	$address = json_decode($json, TRUE);
					
				// 	$requestarr = ['s_address'=>$rideRequest->s_address, 'd_address'=>$address['results'][0]['formatted_address'], 'server_key'=>$siteConfig->server_key];
				...
				// 	$rideRequest->distance = round($location1['distance']/0.621371, 2);
				// 	$rideRequest->d_latitude = $location1['d_latitude'];
				// 	$rideRequest->d_longitude = $location1['d_longitude'];
				// 	$rideRequest->d_address = $address['results'][0]['formatted_address']; //$request->d_address;
				...
				$details = "https://maps.googleapis.com/maps/api/directions/json?origin=".$rideRequest->s_latitude.",".$rideRequest->s_longitude."&destination=".$request->d_latitude.",".$request->d_longitude."&mode=driving&key=AIzaSyDo5-R0tZH0cWEvjvaOXin8fOSKHb_t35Q";
				...
				if(!empty(Auth::guard('provider')->user())){
					$provider = Provider::find(Auth::guard('provider')->user()->id);
					$provider->latitude = $request->d_latitude;
					$provider->longitude = $request->d_longitude;
					$provider->save();
				}
			...
			if (($request->status == 'NOTPAID') && ($rideRequest->payment_mode != 'CASH')) {
				$rideRequest->status = $request->status;
				$rideRequest->paid = 0;  
			}else if (($request->status == 'NOTPAID') && ($rideRequest->payment_mode == 'CASH')) {	
				$rideRequest->status = $request->status;
				$rideRequest->paid = 0;  
			}
			...
			if($request->status == 'DROPPED') {
				...
				// if($request->has('distance') && ($request->distance != null)) {	
				// 	$settings = json_decode(json_encode(Setting::where('company_id', $rideRequest->company_id)->first()->settings_data));
				// 	$transportConfig = $settings->transport;				
				// 	if($transportConfig->unit_measurement=='Kms')
				// 			$rideRequest->distance = round($request->distance/1000,1); //TKM
				// 		else
				// 			$rideRequest->distance = round($request->distance/1609.344,1); //TMi					  
				// }								
				...
				if($request->has('location_points') && $rideRequest->location_points != null) {
					...
					$details = "https://maps.googleapis.com/maps/api/directions/json?origin=".$rideRequest->s_latitude.",".$rideRequest->s_longitude."&destination=".$request->latitude.",".$request->longitude."&waypoints=" . implode($waypoints, '|')."&mode=driving&key=AIzaSyDo5-R0tZH0cWEvjvaOXin8fOSKHb_t35Q";
	...
	public function calculateFare($request, $cflag=0){
		...
		try{
			...
			$location1=$this->getDistanceByAddress($request);	
					
			$settings = json_decode(json_encode(Setting::where('company_id', $request['company_id'])->first()->settings_data));			
			if(isset($request['geofence_id'])) {								
				$ride_city_price = RideCityPrice::where(['geofence_id' => $request['geofence_id'], 'state_id' => $request['state_id'], 'city_id' => $request['city_id']])->where('ride_delivery_vehicle_id', $request['service_type'])->first();
			} else {
				$ride_city_price = RideCityPrice::where('state_id',$request['state_id'])->whereNull('city_id')->where('ride_delivery_vehicle_id', $request['service_type'])->whereNull('city_id')->whereNull('geofence_id')->first();				
			}
			...
			if(!empty($location['errors'])){
				...
				//Below line is commented as client need calculation only in miles
				// if($transportConfig->unit_measurement=='Kms')
				// 	$total_kilometer = round($location['meter']/1000,1); //TKM
				// else
				$total_kilometer = round($location1['distance']/0.621371,1);//TMi	
				...
				$requestarr['is_airport']=$request['is_airport']; 
				if(isset($request['geofence_id'])) $requestarr['geofence_id']=$request['geofence_id'];               
				...
				if($cflag!=0){				
					...
					// Totally wrong logic in old code for calculate fare in Peak hours
					// $peak = 0;
					...
					// 	} else {
					// 		$RideCityPrice = RideCityPrice::where('state_id', $request['state_id'])->where('ride_delivery_vehicle_id', $request['service_type'])->where('company_id', $request['company_id'] )->first();
					// 	}
				...
				$return_data['service_fee']=$price_response['service_fee'];   				
				if(Auth::guard('user')->user()){
					// $return_data['peak']=$peak;    
					// $return_data['peak_percentage']=$peak_percentage;   
					$return_data['peak'] = $price_response['peak_price'];
					$return_data['peak_percentage'] = $price_response['peak_percent'];
	...
	public function calculateFareCalculator($request, $cflag=0){
	...
	public function applyPriceLogic($requestarr,$iflag=0){
		...
		if(isset($requestarr['geofence_id'])) {			
			$ride_city_price = RideCityPrice::where(['geofence_id' => $requestarr['geofence_id'], 'state_id' => $requestarr['state_id'], 'city_id' => $requestarr['city_id']])->where('ride_delivery_vehicle_id', $requestarr['service_type'])->first();
		} else {
			$ride_city_price = RideCityPrice::where('state_id',$requestarr['state_id'])->whereNull('city_id')->where('ride_delivery_vehicle_id', $requestarr['service_type'])->first();			
		}	
		$userCount = User::where(['is_online'=> 0,'status' => 1, 'is_deleted' => 0])->count();
		$providerCount = Provider::where(['providers.is_online'=> 0,'providers.activation_status' => 1, 'providers.is_deleted' => 0])
								->whereHas('providerservice', function($q){
									$q->where('admin_service', '=', 'TRANSPORT');
								})->count();
		$onlineuserCount = User::where(['is_online' => 1,'status' => 1, 'is_deleted' => 0])->count();
		$onlineproviderCount = Provider::where(['providers.is_online' => 1,'providers.activation_status' => 1, 'providers.is_deleted' => 0])
								->whereHas('providerservice', function($q){
									$q->where('admin_service', '=', 'TRANSPORT');
								})->count();
		...
		// Below code is commented due to client needs calculation only in miles
		// if($iflag==0){
		// 	//for estimated fare
		...
		if(!empty($requestarr['is_airport']) && ($requestarr['is_airport'] == 'AIRPORT')){			
			$base_price= ($ride_city_price == null) ? 0 : $ride_city_price->airport_fee; //BP
		} else {
			$base_price= ($ride_city_price == null) ? 0 : $ride_city_price->fixed; //BP
		}		
		//print_r($ride_city_price); exit;
		// if($base_price<$ride_city_price->min_price){
		// 	$base_price = $ride_city_price->min_price;
		// } else {
		...
		if($ride_city_price != null) {			
			if($ride_city_price->calculator == 'MIN') {
				...
				if($base_distance>$total_kilometer){
					...
					//echo $base_price; echo '<br>'; echo $per_kilometer; exit;												
		...
		$peakprice = 0;
		$fn_response['peak_price']=$peakprice;
		$fn_response['user_ratio']=0;
		$fn_response['peak_percent']=0;
		
		if(($userRatio > 0) && ($userRatio <= 14)){
			$peakprice = ($base_price*1.3)/100;
			$fn_response['peak_price']= $peakprice;
			$fn_response['user_ratio']=$userRatio;
			$fn_response['peak_percent']=1.3;
		} else if(($userRatio > 14) && ($userRatio < 35)){
			$peakprice = ($base_price*1.7)/100;
			$fn_response['peak_price']=$peakprice;
			$fn_response['user_ratio']=$userRatio;
			$fn_response['peak_percent']=1.7;
		} else if($userRatio >= 35){
			$peakprice = ($base_price*2.3)/100;
			$fn_response['peak_price']=$peakprice;
			$fn_response['user_ratio']=$userRatio;
			$fn_response['peak_percent']=2.3;
		}

		// \Log::info("***************Peak price for Taxi********************");
		// \Log::info("***************".time()."********************");
		...
		// \Log::info('$peak_percent ='.$fn_response['peak_percent']);
		// \Log::info('$service_fee ='.$ride_city_price->service_fee);
		// \Log::info("***************End of peak price for Taxi********************");
		

		$fn_response['service_fee'] = (double)$ride_city_price->service_fee;
		$price = $ride_city_price->service_fee+$price+$peakprice;		
	...
	public function getLocationDistance($locationarr){
		...
		try{
			...
			$apiurl = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$s_latitude.",".$s_longitude."&destinations=".round($d_latitude,6).",".round($d_longitude, 6)."&mode=driving&sensor=false&units=imperial&key=AIzaSyDo5-R0tZH0cWEvjvaOXin8fOSKHb_t35Q";			
			
			$client = new Client;
			$location = $client->get($apiurl);           
			$location = json_decode($location->getBody(),true);		

			if($location['status'] == 'REQUEST_DENIED'){
				throw new Exception("Request denied by api services to get the distance between source and destination", 1);				
			}
	...
	public function getDistanceByAddress($request){
		// Change address format
	    $formattedAddrFrom    = str_replace(' ', '+', $request['s_address']);
	    $formattedAddrTo     = str_replace(' ', '+', $request['d_address']);
	    
	    // Geocoding API request with start address
	    $geocodeFrom = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address='.$formattedAddrFrom.'&sensor=false&key='.$request['server_key']);
	    ...
	    $geocodeTo = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address='.$formattedAddrTo.'&sensor=false&key='.$request['server_key']);
	    ...
	    return['distance'=>$miles, 's_latitude'=> $latitudeFrom , 's_longitude'=> $longitudeFrom , 'd_latitude'=> $latitudeTo, 'd_longitude'=> $longitudeTo];
	}
	...
	public function invoice($request_id, $toll_price = 0)
	{
		...
		try {                      
			...
			$rideRequest = RideRequest::with('provider')->where(['id' => $request_id])->first();			
			...
			// $transportConfig = $settings->transport;				
			// if($transportConfig->unit_measurement=='Kms')
			// 		$requestarr['kilometer'] = round($rideRequest->distance/1000,1); //TKM
			// 	else
					// $requestarr['kilometer'] = round($rideRequest->distance/1609.344,1); //TMi
			...
			$getfence = GeoFence::where(['id' => $rideRequest->geofence_id])->first();


			$ride_city_price = RideCityPrice::where('state_id',$user->state_id)->whereNull('city_id')->where('ride_delivery_vehicle_id', $requestarr['service_type'])->first();	
			//print_r($ride_city_price); exit;
			if(!empty($getfence) && ($getfence->type == 'AIRPORT')){
				$requestarr['is_airport'] = $getfence->type;		
			} else {
				$requestarr['is_airport'] = '';
			}
			$response = new Ride();         										
			...
			dd($pricedata);
			return false;*/
			$peakprice = $peakpercent = $userratio = 0;
			if(!empty($pricedata)){
				...
				$peakprice = $pricedata['peak_price'];
				$peakpercent = $pricedata['peak_percent'];
				$userratio = $pricedata['user_ratio'];
				$rideRequest->calculator=$pricedata['calculator'];
				$rideRequest->distance=$requestarr['kilometer'];
				$rideRequest->save();				
			...
			if($commission_percentage>0){				
				$Commision = $this->applyPercentage($Distance,$commission_percentage);										
			...
			$Payment->provider_id=Auth::guard('provider')->user()->id;
			...
			// 	if($Peakcharges){
					...
					$peakamount = ($peakprice/100) * $Fixed;
			...
			$Payment->peak_amount = $peakprice;//$peakamount;
			$Payment->peak_percent = $peakpercent;
			$Payment->user_ratio = $userratio;
