...
class ProviderServices { 
			...
			if(!empty($IncomingRequests)){
				...
				for ($i=0; $i < count($IncomingRequests); $i++) {					
					...
					if($admin_service == "TRANSPORT") {
						$details = "https://maps.googleapis.com/maps/api/directions/json?origin=".$IncomingRequests[$i]->request->request->s_latitude.",".$IncomingRequests[$i]->request->request->s_longitude."&destination=".$IncomingRequests[$i]->request->request->d_latitude.",".$IncomingRequests[$i]->request->request->d_longitude."&mode=driving&key=".$this->settings->site->server_key;
						$json = Helper::curl($details);
						$details = json_decode($json, TRUE);
						$jsondata = json_decode($IncomingRequests[$i]->request->request_data);
						$jsondata->distance = !empty($details['routes']) ? $details['routes'][0]['legs'][0]['distance']['value'] : 0;
						$newdata = json_encode($jsondata);
						$IncomingRequests[$i]->request->request_data = $newdata;
					...
					elseif($admin_service == "SERVICE"){
						...
						$details = "https://maps.googleapis.com/maps/api/directions/json?origin=".$request->latitude.",".$request->longitude."&destination=".$jsondata->s_latitude.",".$jsondata->s_longitude."&mode=driving&key=".$this->settings->site->server_key;
						$json = Helper::curl($details);
						$details = json_decode($json, TRUE);
					...
					elseif($admin_service == "ORDER"){
						...
						$jsondata = json_decode($IncomingRequests[$i]->request->request_data);						
						$pickupdata = json_decode($jsondata->pickup_address);
						$deliverydata = json_decode($jsondata->delivery_address);
						$details = "https://maps.googleapis.com/maps/api/directions/json?origin=".$pickupdata->latitude.",".$pickupdata->longitude."&destination=".$deliverydata->latitude.",".$deliverydata->longitude."&mode=driving&key=".$this->settings->site->server_key;
						$json = Helper::curl($details);
						$details = json_decode($json, TRUE);
						$jsondata->distance = !empty($details['routes']) ? $details['routes'][0]['legs'][0]['distance']['value'] : 0;
						$newdata = json_encode($jsondata);
						...
						$notified->request_id = $IncomingRequests[$i]->request_id;
						$notified->admin_service = 'ORDER';
						$notified->save();
		...
		try {        
			...
			if($newRequest->schedule_at != ""){
				...
				if($request->admin_service == "SERVICE" ) {
					...
					$newRequest->order_accepted_at = date('Y-m-d H:i:s');  
					$newRequest->save();
					//Send message to socket
	...
	public function assignProvider($config, $IncomingRequest) {			
		if($config !='' && $config->manual_request == 0){
			...
			$status = 'SEARCHING';
			// if($IncomingRequest->admin_service){
			// 	$status = 'ORDERED';
			// }
			if($IncomingRequest->request->status == $status && $IncomingRequest->request->time_left_to_respond < 0) {
				//if($config->broadcast_request == 1){
	...
	public function assignNextProvider($request_id, $admin_service) 
	{
		...
		try {
			...
			if($nextRequestFilter != null) {
				...
				if($admin_service=='ORDER'){
					(new SendPushNotification)->OrderIncomingRequest($nextRequestFilter->provider_id, $newRequest->admin_service, $newRequest);
				}else{
	...
	public function assignNextProviderOnlyOne($request_id, $admin_service) 
	{
		...
		$storeorder = StoreOrder::find($request_id);
		$Providers = (new UserServices())->nextAvailableDeliveryBoy($storeorder);

		$provider = $Providers->first();
		(new UserServices())->createDeliveryRequest([$Providers->first()], $storeorder, 'ORDER');
		...
		try {
			...
			// if($nextRequestFilter != null) {
				...
				// incoming request push to provider
				(new SendPushNotification)->IncomingRequest($provider->id, strtolower($admin_service));
			//} else {
			// 	$UserRequest->delete();
