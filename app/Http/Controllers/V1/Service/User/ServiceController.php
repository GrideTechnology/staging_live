...
class ServiceController extends Controller
{
	...
	public function recentVisits(Request $request)
	{
		...
		try {
			...
			$item = UserVisit::create([
				'user_id' => $user->id,
				'store_id' => $request->store_id,
			]);

			return Helper::getResponse(['status' => 200, 'message' => 'Recent Visits add successfully']);
		} catch (Exception $e) {
			return Helper::getResponse(['status' => 500, 'message' => 'Something went wrong', 'error' => $e->getMessage()]);
		}

	}
	public function like(Request $request, $item_id)
	{
		return $this->handleLikeOrDislike($request, $item_id, 'like');
	}

	public function dislike(Request $request, $item_id)
	{
		return $this->handleLikeOrDislike($request, $item_id, 'dislike');
	}

	private function handleLikeOrDislike(Request $request, $item_id, $type)
	{
		 
		$this->validate($request, [
			'item_id' => 'required|integer', // Assuming items is the table you want to like/dislike
		]);
		...
		try {
			...
			return Helper::getResponse(['status' => 200, 'message' => 'Item ' . $type . 'd successfully']);
		} catch (Exception $e) {
			return Helper::getResponse(['status' => 500, 'message' => 'Something went wrong', 'error' => $e->getMessage()]);
		}
	}
	...
	public function providerServiceList(Request $request)
	{
		...
		->select(DB::Raw("(3959 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) AS distance"),'id','first_name','picture','rating','city_id','latitude','longitude')
		...
		->where('wallet_balance' ,'>=',$siteConfig->provider_negative_balance);//6371
		if($request->has('name')){
	...
	public function requestStatementHistory(Request $request)
	{
		try {
			$history_status = array('CANCELLED','COMPLETED');
			$serviceRequests = ServiceRequest::where(['company_id' => Auth::user()->company_id,'is_deleted' => 0])
	...
	public function destroy(Request $request, $id){
		try {
			$datum = ServiceRequest::findOrFail($id);        
			$datum->is_deleted = 1;
	...
	public function webcompanyproviderservice(Request $request, $id){
		try{
			$document = ProviderAgreeDocument::select('provider_agree_document.*', 'company_documents.name')->join('company_documents', 'company_documents.id', '=', 'provider_agree_document.document_id')->where('user_id', $id)->get();
