...
$router->group(['middleware' => 'auth:user'], function($app) {
	...
	$app->post('/items/{item}/like', 'V1\Service\User\ServiceController@like');
	$app->post('/items/{item}/dislike', 'V1\Service\User\ServiceController@dislike');
	$app->post('/recent_visits', 'V1\Service\User\ServiceController@recentVisits');
